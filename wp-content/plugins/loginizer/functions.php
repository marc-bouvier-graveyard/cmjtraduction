...
// Get the value of option from lz_options table
function lz_get_option($option_name, $default = ''){
	
	global $wpdb;
	
	$result = lz_selectquery("SELECT `option_value` FROM `".$wpdb->prefix."lz_options` WHERE `option_name` = '".$option_name."';");
...
// Update the value of an option in lz_options table
function lz_update_option($option_name, $value){
	
	global $wpdb;
	
	$result = $wpdb->query("INSERT INTO `".$wpdb->prefix."lz_options` SET `option_name` = '".$option_name."', `option_value` = '".$value."', `updated` = '".time()."' ON DUPLICATE KEY UPDATE `option_value` = '".$value."', `updated` = '".time()."';");
